...
class glowbit(colourFunctions, colourMaps):
    ...
    ## @brief Calculates an estimate for the total power draw given the current display data.
    #  Use as a general guide only, error range is around 10-20%.
    #
    # The estimate is a 4th order polynomical interpolation given measurements of white brightness.
    # The power consumption of pure colours will tend to be under-estimated by up to about 10-20%.
    #
...
class stick(glowbit):
    ...
    # 
    # The pulse returning "down" the stick is drawn with a 3-pixel list of colours.
    # The first and last are coloured with the "Rainbow" colour map, changing colour with pixel index, while the middle is white.
    # 
...
class matrix8x8(glowbitMatrix):
    ...
    # \param brightness The relative brightness of the LEDs. Colours drawn to the internal buffer should be in the range [0,255] and the brightness parameter scales this value before drawing to the physical display. If brightness is an integer it should be in the range [0,255]. If brightness is floating point it is assumed to be in the range [0,1.0].
    # \param mapFunction A function pointer to a custom pixel mapping function. Only required if mapping pixels to non-standard tiling arrangements.
    # \param rateLimitFPS The maximum frame rate of the display in frames per second.
    #  The pixelsShow() function blocks to enforce this limit. This argument defaults to -1 to allow
    #  rateLimitCharactersPerSecond to preference this parameter if it is not set.
    #  If neither rateLimitFPS or rateLimitCharactersPerSecond are set the limit is set to 30 FPS.
    # \param rateLimitCharactersPerSecond If given a positive value the display update rate is set to display this
    #  many characters of scrolling text per second. A value of 1 is fast, but readable. This value can be fractional (eg: 0.5).
    # \param sm (Raspberry Pi Pico only) The PIO state machine to generate the GlowBit data stream. Each connected GlowBit display chain requires a unique state machine. Valid values are in the range [0,7].
    ...
    ## @brief Prints a string of text to a tiled GlowBit Matrix 8x8 display,
    #    automatically wrapping to new lines as required. Each character occupies an 8x8 pixel area.
    #
